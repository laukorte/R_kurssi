...
R:n on komentopohjainen ohjelma, jonka käyttöliittymänä toimii komentoikkuna (Console). Siihen kirjoitetaan komennot ja komentojen tulokset tulostuvat myös komentoikkunaan. R:ää voidaan pitää "interaktiivisena laskukoneena" (Rajala 2001) ja komentojen lisäksi konsolia voidaankin käyttää laskimena.
...
Edellä annettu komento laski yhteen luvut 1 ja 2 ja tulosti näiden summan. Näin toimien tulos ei kuitenkaan jää muistiin, vaan se pitää tallentaa erikseen. Arvon tallettaminen muuttujaan tapahtuu sijoitusoperaattorilla "<-". Luodaan seuraavaksi muuttuja nimeltään *luku*, jolle annetaan arvo yksi. Tämän jälkeen tulostetaan muuttujan 
sisältö kirjoittamalla muuttujan nimi ja ajamalla tämä komento. Monessa muussa ohjelmointikielessä muuttujan arvon tulostamiseen pitäisi kutsua erikseen tulostusfunktiota (kuten print), mutta R:ssä tämä ei ole tarpeellista.
...
HUOM! Jos samaan muuttujaan talletaan jokin muu luku tai tulos, R ei kysele lupia, vaan tekee mitä pyydetään. Ole siis tarkkana, kun teet muuttujia!
...
Muuttuja voidaan kopioida yksinkertaisesti sijoittamalla vanha muuttuja uuteen muuttujaan.
...
Komentoja voidaan käyttää myös funktion sisällä. Seuraavassa luodaan ensin tuhat havaintoa normaalijakautuneesta muuttujasta, jonka keskiarvo on 100 ja keskihajonta on 15. Sen jälkeen lasketaan muuttujan keskivirhe (= sd / n^0.5) käyttämällä hyväksi komentoja.
...
Edellä komennot on annettu komentoikkunan kautta. Komennot voidaan suorittaa myös 
...
Luodaan vektori, jossa on luvut 1, 3 ja 10 ja talletetaan se muuttujaan *v1*.
```{r}
v1 <- c(1, 3, 10)
v1
```

Edellä luotujen muuttujien tapaan myös vektoriin voidaan viitata pelkästään sen nimellä.
